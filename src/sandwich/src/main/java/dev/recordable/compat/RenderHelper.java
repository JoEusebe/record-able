 ...
 * <p>The sandwich build targets a wide range (1.20.5-1.21.11). A previous
...
public final class RenderHelper {
    ...
    private static volatile boolean orderedTextResolved;
    private static Method orderedTextMethod;
    private static boolean orderedTextHasShadowArg;
    ...
    private static void resolveOrderedText() {
        Method sixArg = null;   // (TextRenderer, OrderedText, int, int, int, boolean)
        Method fiveArg = null;  // (TextRenderer, OrderedText, int, int, int)
        for (Method m : DrawContext.class.getMethods()) {
            Class<?>[] p = m.getParameterTypes();
            if (p.length == 6 && p[0] == TextRenderer.class && p[1] == OrderedText.class
                    && p[2] == int.class && p[3] == int.class && p[4] == int.class && p[5] == boolean.class) {
                sixArg = m;
            } else if (p.length == 5 && p[0] == TextRenderer.class && p[1] == OrderedText.class
                    && p[2] == int.class && p[3] == int.class && p[4] == int.class) {
                if (fiveArg == null || m.getName().contains("51433") || m.getName().toLowerCase().contains("shadow")) {
                    fiveArg = m;
                }
            }
        }
        if (sixArg != null) {
            orderedTextMethod = sixArg;
            orderedTextHasShadowArg = true;
        } else {
            orderedTextMethod = fiveArg;
            orderedTextHasShadowArg = false;
        }
        orderedTextResolved = true;
    }
    ...
    /** Renders an {@link OrderedText}. Never throws - degrades to a no-op on failure. */
    public static void drawText(DrawContext context, TextRenderer textRenderer,
                                OrderedText text, int x, int y, int color, boolean shadow) {
        if (!orderedTextResolved) {
            resolveOrderedText();
        }
        try {
            Method m = orderedTextMethod;
            if (m == null) {
                return;
            }
            if (orderedTextHasShadowArg) {
                m.invoke(context, textRenderer, text, x, y, color, shadow);
            } else {
                m.invoke(context, textRenderer, text, x, y, color);
            }
        } catch (Throwable ignored) {
     ...
     * {@code DrawContext} wrapped-text method.
     *
     * <p>Older code called {@code DrawContext.drawWrappedTextWithShadow(...)}
     * (intermediary {@code method_65179}), which only exists in newer 1.21.x
     * builds and threw {@link NoSuchMethodError} on the older versions this
     * sandwich build also supports (1.20.5+), crashing the settings screen.</p>
     *
     * <p>This implementation wraps manually using {@code TextRenderer.getWidth}
     * (stable across every supported version) and draws each line through the
     * hardened {@link #drawText} above. Never throws.</p>
     *
     ...
     * Always {@code true} for this sandwich build (targets 1.20.5-1.21.11).
     */
