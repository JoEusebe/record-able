...
public final class ModCompatibilityChecker {
     ...
     *
     * @param client the active {@link MinecraftClient} instance
     */
    public static void warnPlayerIfConflicts(MinecraftClient client) {
        ...
        if (replayCoexisting) {
            client.player.sendMessage(
                    Text.literal(ReplayCompatBridge.getCoexistenceMessage()),
                    false);
        }
        ...
        client.player.sendMessage(
                Text.literal("§e[Record-able] §cWarning: conflicting mod(s) detected!"),
                false);
        for (String conflict : hardConflicts) {
            String brief = conflict.replace("[Record-able] Mod conflict detected: ", "");
            client.player.sendMessage(
                    Text.literal("§7  • " + brief),
                    false);
        }
        client.player.sendMessage(
                Text.literal("§eRecording may have A/V sync issues or instability. " +
     ...
     */
    public static boolean detectReplayPlayback(MinecraftClient client) {
        try {
            if (client == null) {
                return false;
            }
            ...
            if (ReplayCompatBridge.isReplayClass(client.getServer())) {
                return true;
            }
            return false;
        } catch (Throwable t) {
