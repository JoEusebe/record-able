...
public final class ScreenCapture implements AutoCloseable {
    ...
    //   MAIN_TEXTURE (3): read the main render target's COLOR TEXTURE via glGetTexImage
    //                     instead of glReadPixels. This mirrors how vanilla Minecraft
    //                     takes F2 screenshots, which is the ONLY readback path proven
    //                     to work on Pojav/Zalith GLES layers (where glReadPixels
    //                     silently returns all-black). Default first source on Android.
    ...
    /**
     * Capture a frame synchronously (blocking glReadPixels), bypassing the async PBO path.
     * Used by the filter pipeline, which needs a deterministically-paired clean (pre-HUD)
     * frame and full (post-HUD) frame from the same render pass - async PBO frames lag by
     * one frame and cannot be paired reliably.
     *
    ...
    private CapturedFrame captureSynchronously(int nativeWidth, int nativeHeight) {
        ...
        try {
            ...
            try {
                ...
                // capturing before Gui.render() - at that point, the main FBO has
    ...
    private CapturedFrame captureViaDesktopWindow(Minecraft client) {
    ...
    /**
     * Returns the GL id of Minecraft's main render target, or {@code -1} when it
     * cannot be determined. Uses reflection so the same source compiles across
     * mapping renames between Minecraft versions.
     * 
     * <p><b>MC 26.2+ Note:</b> Minecraft's new GPU abstraction (GpuTexture/GpuTextureView)
     * replaced the direct {@code frameBufferId} field with a texture-based architecture.
     * FBOs are now cached and created on-demand by {@code FrameBufferCache}, making direct
     * FBO ID access complex. This method tries the new API pattern (casting colorTexture to
     * GlTexture and accessing its associated FBO cache), but falls back to -1 if unsuccessful.
     * When -1 is returned, {@link #resolveReadFbo} uses the currently-bound draw FBO, which
     * is typically the RenderTarget's FBO during rendering and works correctly for pixel capture.
     */
    private static int mainFramebufferId() {
        try {
            ...
            Object renderTarget = client.gameRenderer.mainRenderTarget();
            if (renderTarget == null) {
                return -1;
            }
            
            // MC 26.2+: Try new GpuTexture API - get color texture and attempt to find associated FBO
            try {
                java.lang.reflect.Method getColorTexture = renderTarget.getClass().getMethod("getColorTexture");
                Object colorTexture = getColorTexture.invoke(renderTarget);
                if (colorTexture != null) {
                    // If it's a GlTexture, try to get the GL texture ID (not FBO, but kept for potential future use)
                    java.lang.reflect.Method glIdMethod = colorTexture.getClass().getMethod("glId");
                    Object glId = glIdMethod.invoke(colorTexture);
                    if (glId instanceof Integer texId && texId > 0) {
                        // We have the texture ID, but not the FBO ID directly.
                        // In MC 26.2, FBOs are cached. Rather than trying to navigate the complex
                        // FrameBufferCache, we return -1 to fall back to activeDrawFbo, which
                        // is the RenderTarget's FBO when rendering is active.
                        return -1;
                    }
                }
            } catch (NoSuchMethodException | SecurityException ignored) {
                // MC < 26.2 or unexpected API, fall through to legacy field access
            }
            
            // MC < 26.2: Try legacy direct frameBufferId field access
            for (String fieldName : new String[]{"frameBufferId", "fbo", "field_1476"}) {
                try {
                    java.lang.reflect.Field f = renderTarget.getClass().getField(fieldName);
                    Object v = f.get(renderTarget);
    ...
    /**
     * Captures a frame by downloading the main render target's COLOR TEXTURE via
     * {@code glGetTexImage}, instead of reading a framebuffer with {@code glReadPixels}.
     * This mirrors how vanilla Minecraft saves F2 screenshots and is the ONLY readback
     * path that returns real pixels on Pojav/Zalith/MobileGlues GLES translation layers,
     * where {@code glReadPixels} silently yields all-black.
     *
    ...
    /**
     * Returns the GL texture id of the main render target's color texture, or {@code -1}
     * when it cannot be determined. On MC 26.2+ this uses the GpuTexture API
     * ({@code mainRenderTarget().getColorTexture().glId()}); older versions fall back to
     * the {@code colorTextureId}/{@code colorAttachment} field.
     */
    private static int mainColorAttachmentTexture() {
        try {
            ...
            // MC 26.2+: GpuTexture API - getColorTexture().glId()
            try {
                java.lang.reflect.Method getColorTexture = renderTarget.getClass().getMethod("getColorTexture");
                Object colorTexture = getColorTexture.invoke(renderTarget);
                if (colorTexture != null) {
                    for (String glIdName : new String[]{"glId", "getGlId", "id"}) {
                        try {
                            java.lang.reflect.Method glIdMethod = colorTexture.getClass().getMethod(glIdName);
                            Object glId = glIdMethod.invoke(colorTexture);
                            if (glId instanceof Integer texId && texId > 0) {
                                return texId;
                            }
                ...
                // MC < 26.2, fall through to legacy field access
            }
            // MC < 26.2: legacy direct colorTextureId field access
            for (String fieldName : new String[]{"colorTextureId", "colorAttachment", "field_1476", "field_1480"}) {
                try {
                    java.lang.reflect.Field f = renderTarget.getClass().getField(fieldName);
                    Object v = f.get(renderTarget);
                    if (v instanceof Integer i && i > 0) {
    ...
    /**
     * Best-effort lookup of the main render target's backing texture dimensions
     * ({@code width}/{@code height} / {@code viewWidth}/{@code viewHeight}). Returns
     * {@code null} when the fields cannot be resolved (caller uses the window size).
     */
    private static int[] mainFramebufferTextureSize() {
        try {
            ...
            Integer w = readIntField(renderTarget, new String[]{"width", "viewWidth", "field_1480"});
            Integer h = readIntField(renderTarget, new String[]{"height", "viewHeight", "field_1477"});
     ...
     * {@code {width, height}} or {@code null} when it cannot be determined.
     * Callers must treat {@code null} as "unknown", never as an error.
     */
    public static int[] currentRenderTargetSize() {
        try {
            ...
            return CaptureDiagnostics.readRenderTargetSize(client.gameRenderer.mainRenderTarget());
        } catch (Throwable ignored) {
            return null;
        }
    }
