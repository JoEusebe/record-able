...
public final class AutoRecordManager {
     ...
     * running and start a grace window. {@link #resolveTransferState(Minecraft)} decides,
    ...
    public void scheduleAutoRecording(Minecraft client, String reason) {
        RecordableConfig config = RecordableConfig.get();
    ...
    private void handleGameStartTrigger(Minecraft client) {
        if (gameStartHandled) {
            return;
        }
     ...
     * </ul>
     */
    private void resolveTransferState(Minecraft client) {
        try {
            RecordingManager recordingManager = RecordingManager.getInstance();
            ...
            Screen currentScreen = VersionHelper.currentScreen(client);
            if (isTerminalLeaveScreen(currentScreen)) {
    ...
    private void finishLeave(Minecraft client) {
        awaitingTransfer = false;
        transferTicksRemaining = -1;
        sawWorldDuringRecording = false;
        cancelScheduledAutoRecord();
    ...
    private void triggerStartNow(Minecraft client) {
    ...
    private static void stopIfRecording(Minecraft client, String reason) {
        Minecraft activeClient = resolveClient(client);
        if (activeClient != null && !activeClient.isSameThread()) {
