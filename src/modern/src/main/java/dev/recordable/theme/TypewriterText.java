...
public final class TypewriterText {
    ...
    public int render(GuiGraphicsExtractor ctx, Font font, int x, int y, int color) {
        ...
        ctx.text(font, visible, x, y, color, true);

        // Cursor blink
        if (!completed || (System.currentTimeMillis() / 500) % 2 == 0) {
            int cursorX = x + font.width(visible);
        ...
        if (ThemeEngine.get().preset() == ThemePreset.VHS && !completed) {
            if (elapsed % 400 < 30) {
                ...
                ctx.fill(x, y - 1, x + font.width(visible) + 2, y + 10, flickerAlpha << 24);
            }
        }
    ...
    public static void renderFlickerText(GuiGraphicsExtractor ctx, Font font, String text, int x, int y, int baseColor) {
