...
public final class ThemedPanel {
    ...
    public static void drawPanel(GuiGraphicsExtractor ctx, int left, int top, int right, int bottom) {
    ...
    public static void drawFilmPanel(GuiGraphicsExtractor ctx, int left, int top, int right, int bottom) {
        ThemeColors colors = ThemeEngine.get().colors();
        int stripW = 12;
        // Main background
    ...
    public static void drawSectionHeader(GuiGraphicsExtractor ctx, Font font, String text, int x, int y, int maxWidth) {
        ThemeColors colors = ThemeEngine.get().colors();
        ...
        // Header text
        ctx.text(font, text, x, y, colors.headerText, true);
        ...
        if (preset == ThemePreset.VHS || preset == ThemePreset.NEON) {
            ctx.text(font, "▌", x - 8, y, colors.accent, true);
        }
        if (preset == ThemePreset.GALAXY) {
            ctx.text(font, "✶", x - 10, y, colors.accentHover, true);
        }
        // Cinema theme: small "★" before header
        if (preset == ThemePreset.CINEMA) {
            ctx.text(font, "★", x - 10, y, colors.accent, true);
        }
    }

    /** Thin horizontal divider line. */
    public static void drawDivider(GuiGraphicsExtractor ctx, int x, int y, int width) {
        ThemeColors colors = ThemeEngine.get().colors();
    ...
    /** Themed scrollbar. */
    public static void drawScrollbar(GuiGraphicsExtractor ctx, int barLeft, int barTop, int barBottom, int thumbTop, int thumbHeight) {
        ThemeColors colors = ThemeEngine.get().colors();
    ...
    public static void drawReelLoading(GuiGraphicsExtractor ctx, int centerX, int centerY, int radius) {
        ThemeColors colors = ThemeEngine.get().colors();
    ...
    public static void drawVhsStatusBadge(GuiGraphicsExtractor ctx, Font font, String text, int x, int y, boolean blink) {
        ThemeColors colors = ThemeEngine.get().colors();
        ...
        if (blink) {
            ...
            ctx.text(font, text, x, y, textColor, true);
        } else {
            ctx.text(font, text, x, y, colors.textPrimary, true);
        }
    }

    /** Draw a category tab-style row for settings sections. */
    public static void drawCategoryTab(GuiGraphicsExtractor ctx, Font font, String label, int x, int y, int width, boolean selected) {
        ThemeColors colors = ThemeEngine.get().colors();
        ...
        ctx.text(font, label, x + 6, y + 4, selected ? colors.textPrimary : colors.textSecondary, true);
    }
}
