...
public final class PendingRendersScreen extends Screen {
    ...
    public PendingRendersScreen(Screen parent) {
        super(Component.literal("Pending Renders"));
        this.parent = parent;
    }
    ...
    private void rebuildPanelWidgets() {
        ...
        // Render All (batch)
        Button renderAllBtn = Button.builder(
                Component.literal("Render All"),
                button -> renderAll()
        ).bounds(this.panelLeft + 12, topBtnY, btnW, btnH).build();
        renderAllBtn.active = !sessions.isEmpty();
        this.addRenderableWidget(renderAllBtn);
        ...
        for (int i = 0; i < sessions.size(); i++) {
            ...
            int btnRowY = rowY + 14;
            this.addRenderableWidget(Button.builder(
                    Component.literal("Render Now"),
                    button -> renderSession(sessionId)
            ).bounds(rightEdge - 150, btnRowY, 84, 18).build());

            this.addRenderableWidget(Button.builder(
                    Component.literal("Delete"),
                    button -> deleteSession(sessionId)
            ).bounds(rightEdge - 60, btnRowY, 56, 18).build());
        }
        ...
        this.addRenderableWidget(Button.builder(
                ...
                button -> onClose()
        ).bounds((this.width - closeW) / 2, this.panelBottom - 28, closeW, 20).build());
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.text(this.font, Component.literal(sessions.size() + " pending session(s)"),
                textLeft, this.panelTop + 22, TEXT_COLOR, true);

        if (sessions.isEmpty()) {
            context.centeredText(this.font, Component.literal("No pending renders."),
            ...
            for (int i = 0; i < sessions.size(); i++) {
                ...
                context.text(this.font, Component.literal(title), textLeft, rowY + 2, HEADER_COLOR, true);
                ...
                context.text(this.font, Component.literal(detail), textLeft, rowY + 15, SUBTEXT_COLOR, true);
            }
        }
