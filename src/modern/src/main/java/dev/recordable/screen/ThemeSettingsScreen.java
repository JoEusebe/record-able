...
public final class ThemeSettingsScreen extends Screen {
    ...
    public ThemeSettingsScreen(Screen parent) {
        super(Component.translatable("screen.recordable.theme.title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        addRenderableWidget(CycleButton.create(wLeft, y, wWidth, WIDGET_HEIGHT,
                Component.literal("Theme: " + config.uiTheme.displayName),
                button -> {
                    config.uiTheme = config.uiTheme.next();
                    ThemeEngine.get().applyPreset(config.uiTheme);
                    config.save();
                    if (this.minecraft != null) {
                        this.minecraft.setScreenAndShow(new ThemeSettingsScreen(this.parent));
                    }
                }, button -> {
        ...
        if (config.uiTheme == ThemePreset.GALAXY) {
            addRenderableWidget(CycleButton.create(wLeft, y, wWidth, WIDGET_HEIGHT,
                    Component.literal(galaxyGlowButtonLabel(config.uiGalaxyGlowIntensity)),
                    button -> {
                        config.uiGalaxyGlowIntensity = cycleGalaxyGlow(config.uiGalaxyGlowIntensity, true);
                        ThemeEngine.get().loadFromConfig();
                        config.save();
                        button.setMessage(Component.literal(galaxyGlowButtonLabel(config.uiGalaxyGlowIntensity)));
                    }, button -> {
        ...
        // Reset to defaults
        addRenderableWidget(ThemedButton.create(wLeft, y, wWidth, WIDGET_HEIGHT,
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        if (titleAnim != null) {
            titleAnim.render(context, this.font, panelLeft + 10, panelTop + 8, colors.headerText);
        }
        ...
        context.text(this.font, line1, panelLeft + 10, this.descriptionY, colors.textMuted, true);
        String remainder = description.length() > line1.length() ? description.substring(line1.length()).trim() : "";
        if (!remainder.isEmpty()) {
            context.text(this.font, fitText(remainder, descMaxW), panelLeft + 10, this.descriptionY + 10, colors.textMuted, true);
        }
    ...
    private void addThemeToggle(int x, int y, int width, RecordableConfig config, ThemePreset.ThemeToggle toggle) {
        addRenderableWidget(ThemedToggle.create(x, y, width, WIDGET_HEIGHT,
    ...
    private void renderPreview(GuiGraphicsExtractor context) {
        ...
        // Section 1: Title
        // ═══════════════════════════════════════════
        ThemedPanel.drawSectionHeader(context, this.font, "Live Preview", px, py, pw);
        py += 22;

        // ═══════════════════════════════════════════
        // Section 2: Sample settings text
        // ═══════════════════════════════════════════
        ThemedPanel.drawSectionHeader(context, this.font, "Recording Settings", px, py, pw);
        py += 18;

        context.text(this.font, "Resolution: " + previewResolution(), px + 8, py, colors.textPrimary, true);
        py += 12;
        context.text(this.font, "FPS: " + previewFps(), px + 8, py, colors.textPrimary, true);
        py += 12;
        context.text(this.font, "Encoder: " + previewEncoderName(), px + 8, py, colors.textSecondary, true);
        py += 18;
        ...
        // ═══════════════════════════════════════════
        context.text(this.font, "Quick Keys:", px + 6, py, colors.textMuted, true);
        py += 14;
        ...
        for (String label : keyLabels) {
            maxLabelW = Math.max(maxLabelW, this.font.width(label));
        }
        ...
        for (int k = 0; k < keyLabels.length; k++) {
            ...
            int keyTextW = this.font.width(keyLabels[k]);
            int keyColor = "Not Bound".equals(keyLabels[k]) ? colors.textMuted : colors.accent;
            context.text(this.font, keyLabels[k], bx + (badgeW - keyTextW) / 2, py + 2, keyColor, true);
            // Description text beside badge
            int descX = bx + badgeW + 6;
            int descMaxW = Math.max(24, previewRight - descX - 8);
            context.text(this.font, fitText(keyDescs[k], descMaxW), descX, py + 2, colors.textPrimary, true);
            py += badgeH + 4;
        }
        py += 6;
