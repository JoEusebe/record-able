...
public final class FfmpegDownloadScreen extends Screen implements FfmpegBundleManager.ProgressListener {
    ...
    public FfmpegDownloadScreen(Screen parent) {
        super(Component.literal("Record-able: FFmpeg Setup"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        if (alreadyAvailable) {
            // Single "Close" button.
            this.cancelButton = Button.builder(Component.literal("Close"),
                    btn -> onClose())
                    .bounds(centerX - btnW / 2, btnY, btnW, btnH)
                    .build();
            ...
            this.downloadButton = Button.builder(Component.literal(dlLabel),
                    btn -> startDownload())
                    .bounds(centerX - btnW - 4, btnY, btnW, btnH)
            ...
            this.cancelButton = Button.builder(Component.literal(isDownloading ? "Hide" : "Cancel"),
                    btn -> onClose())
                    .bounds(centerX + 4, btnY, btnW, btnH)
                    .build();
            this.addRenderableWidget(this.cancelButton);
        } else {
            // Manual-only (Android or unknown).
            this.openFolderButton = Button.builder(Component.literal("Open FFmpeg Folder"),
        ...
        if (!isDownloading) {
            ...
            this.addRenderableWidget(Button.builder(Component.literal("Test FFmpeg"), b -> runTest())
                    .bounds(bx, row2Y, third, btnH)
                    .tooltip(net.minecraft.client.gui.components.Tooltip.create(Component.literal(
                            "Re-probe every FFmpeg location and show exactly what was tried and why each failed.")))
                    .build());
            this.addRenderableWidget(Button.builder(Component.literal("Paste FFmpeg Path"), b -> pastePathFromClipboard())
                    .bounds(bx + third + gap, row2Y, third, btnH)
                    .tooltip(net.minecraft.client.gui.components.Tooltip.create(Component.literal(
                            "Set the FFmpeg path from your clipboard, e.g.\n/data/data/com.termux/files/usr/bin/ffmpeg\nThen it is tested immediately.")))
                    .build());
            this.addRenderableWidget(Button.builder(Component.literal("Copy Termux Cmd"), b -> copyTermuxCommand())
                    .bounds(bx + (third + gap) * 2, row2Y, totalW - (third + gap) * 2, btnH)
                    .tooltip(net.minecraft.client.gui.components.Tooltip.create(Component.literal(
    ...
    private void startDownload() {
        ...
        if (this.downloadButton != null) {
            this.downloadButton.setMessage(Component.literal("Downloading…"));
    ...
    private void openBundleFolder() {
        try {
            ...
            net.minecraft.util.Util.getPlatform().openFile(FfmpegBundleManager.getBundleDirectory().toFile());
        } catch (Exception e) {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            context.text(this.font, Component.literal(prefix + line.text), textLeft, y, line.color, true);
        }
    ...
    private java.util.List<String> wrapToWidth(String text, int maxWidth) {
        ...
        if (maxWidth <= 8 || this.font.width(text) <= maxWidth) { out.add(text); return out; }
        ...
        for (String w : words) {
            ...
            while (this.font.width(curIndent + word) > maxWidth && word.length() > 1) {
                if (hasWord) {
                ...
                while (fit < word.length()
                        && this.font.width(curIndent + word.substring(0, fit + 1)) <= maxWidth) {
                    fit++;
                }
            ...
            if (!hasWord) {
                ...
                hasWord = true;
            } else if (this.font.width(line + " " + word) <= maxWidth) {
