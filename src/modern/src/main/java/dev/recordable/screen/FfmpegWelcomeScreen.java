...
public final class FfmpegWelcomeScreen extends Screen {
    ...
    public FfmpegWelcomeScreen(Screen parent) {
        super(Component.literal("FFmpeg Setup Required"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        if (autoSupported) {
            ...
            addRenderableWidget(ThemedButton.create(startX, buttonY, buttonWidth, buttonHeight,
                    Component.literal("Download FFmpeg"), button -> {
                RecordableConfig.get().ffmpegFirstRunShown = true;
                RecordableConfig.get().save();
                if (this.minecraft != null) {
                    this.minecraft.setScreenAndShow(new FfmpegDownloadScreen(this.parent));
                }
            }));

            addRenderableWidget(ThemedButton.create(startX + buttonWidth + gap, buttonY, buttonWidth, buttonHeight,
                    Component.literal("Dismiss"), button -> dismiss()));
        } else {
            // Only show "Dismiss" button (auto-download not supported)
            int centerX = (this.width - buttonWidth) / 2;
            addRenderableWidget(ThemedButton.create(centerX, buttonY, buttonWidth, buttonHeight,
                    Component.literal("Dismiss"), button -> dismiss()));
        }
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        // Title
        context.centeredText(this.font, Component.literal("FFmpeg Setup Required"),
        ...
        for (String line : lines) {
            ...
            for (String wrappedLine : wrapped) {
                ...
                context.text(this.font, Component.literal(wrappedLine), textX, textY, color);
                textY += lineHeight;
            }
        }
    }
