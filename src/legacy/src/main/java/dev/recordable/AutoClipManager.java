...
public final class AutoClipManager {
     ...
     * (since {@code getLastHurtMob()} is not available client-side in 1.20.x).
     */
     ...
     * LivingEntity subclasses like armor stands, etc. This prevents destroying decorative
     * entities from falsely triggering kill clips.
     */
    ...
    /** Updates {@link #ticksSincePlayerSwing} once per tick from the player's combat-action state. */
    ...
    public void onPlayerAttackEntity(net.minecraft.entity.Entity target) {
        ...
        if (living == client() || living.isRemoved()) return;
    ...
    private static net.minecraft.entity.LivingEntity client() {
    ...
    public void onClientTick(MinecraftClient client) {
        ...
        if (config.autoClipOnKill || config.autoClipOnPlayerKill) {
            scanForPlayerProjectileHits(client);
    ...
    /**
     * Best-effort detection of projectile kills (thrown trident / "spear", arrows, etc.).
     * Fabric's melee attack callback never fires for ranged weapons, so kills made with a
     * thrown trident or a bow were previously never tracked. Here we look for persistent
     * projectiles owned by the local player and, when one is touching a living entity,
     * begin tracking that entity so its imminent death fires the kill clip.
     */
    private void scanForPlayerProjectileHits(MinecraftClient client) {
        try {
            if (client.player == null || client.world == null) return;
            // Keep an already-tracked, still-alive target.
            net.minecraft.entity.LivingEntity current = killTrackTarget != null ? killTrackTarget.get() : null;
            if (current != null && current.isAlive()) return;
            net.minecraft.entity.LivingEntity processed = killLastProcessed != null ? killLastProcessed.get() : null;

            for (net.minecraft.entity.Entity e : client.world.getEntities()) {
                // Use the ProjectileEntity base so this also covers thrown tridents/"spears",
                // arrows and most modded thrown weapons (which extend the vanilla projectile
                // hierarchy), not just PersistentProjectileEntity.
                if (!(e instanceof net.minecraft.entity.projectile.ProjectileEntity proj)) continue;
                // ATTRIBUTION: only projectiles OWNED by the local player count. This is a hard,
                // reliable attribution signal for ranged kills (arrows, tridents, snowballs, etc.).
                if (proj.getOwner() != client.player) continue;
                net.minecraft.util.math.Box hitBox = proj.getBoundingBox().expand(1.0);
                for (net.minecraft.entity.Entity victim : client.world.getOtherEntities(client.player, hitBox)) {
                    if (!(victim instanceof net.minecraft.entity.LivingEntity living)) continue;
                    if (living == processed) continue;
                    if (living.isRemoved()) continue;
                    // Only treat it as a real hit (not a projectile merely flying past) if the
                    // entity is actively taking damage or already dying.
                    boolean hurtOrDying = living.hurtTime > 0 || living.getHealth() <= 0.0f;
                    if (!hurtOrDying) continue;
                    beginTracking(living, "player-owned projectile hit");
                    return;
                }
            }
        } catch (Throwable t) {
            RecordableMod.LOGGER.debug("Projectile kill scan failed.", t);
        }
    }
