...
public final class AudioHelpScreen extends Screen {
    ...
    public AudioHelpScreen(Screen parent) {
        super(Text.translatable("screen.recordable.audio_help.title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        this.addDrawableChild(ButtonWidget.builder(
                Text.translatable("screen.recordable.audio_help.back"),
                button -> close()
        ).dimensions((this.width - buttonWidth) / 2, this.panelBottom - 28, buttonWidth, 20).build());
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < helpLines.size(); i++) {
            ...
            if (line.bold) {
                RenderHelper.drawText(context, this.textRenderer, Text.literal("§l" + line.text), textLeft, y, line.color);
            } else {
                RenderHelper.drawText(context, this.textRenderer, Text.literal(line.text), textLeft, y, line.color);
            }
        }
