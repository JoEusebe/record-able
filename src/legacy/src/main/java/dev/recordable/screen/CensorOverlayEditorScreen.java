...
/**
 * V1-0.08 full-screen live censor editor.
 *
...
public final class CensorOverlayEditorScreen extends Screen {
    ...
    public CensorOverlayEditorScreen(Screen parent) {
        super(Text.literal("Censor Editor"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        addDrawableChild(ThemedButton.create(x, y, bw, WIDGET_HEIGHT,
                Text.literal("Add Bar"), b -> addRegion()));
        x += bw + gap;
        addDrawableChild(ThemedButton.create(x, y, bw, WIDGET_HEIGHT,
                Text.literal("Remove"), b -> removeSelected()));
        x += bw + gap;
        addDrawableChild(ThemedButton.create(x, y, bw, WIDGET_HEIGHT,
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        // Draw widgets and our overlay; no dimmed background (game stays visible).
        ...
        if (config != null) {
            ...
            for (int i = 0; i < rs.size(); i++) {
                ...
                if (r.style == CensorRegion.Style.GRADIENT) {
                    context.fillGradient(x0, y0, x1, y1, 0xCC000000 | (r.color & 0xFFFFFF),
                            0xCC000000 | (r.colorEnd & 0xFFFFFF));
                } else {
                ...
                RenderHelper.drawText(context, tr, Text.literal("\u25CF " + tag),
            ...
            context.drawCenteredTextWithShadow(tr, Text.literal(info), this.width / 2, 8, 0xFFFFFFFF);
            String count = rs.isEmpty() ? "No censor bars yet" : ("Bars: " + rs.size());
            context.drawCenteredTextWithShadow(tr, Text.literal(count), this.width / 2, 20, 0xFFB0B0B0);
            if (!config.streamerModeEnabled) {
                context.drawCenteredTextWithShadow(tr,
                        Text.literal("Streamer Mode is OFF - bars are saved but not shown in-game"),
    ...
    public void renderBackground(DrawContext context, int mouseX, int mouseY, float delta) {
        // When a world is loaded the live game shows through (no dimming) so bars can be
        // positioned over real gameplay. With no world behind (e.g. opened from the title
        // screen via Mod Menu) we MUST draw a backdrop, otherwise the framebuffer is never
        // cleared between frames and GUI draws smear/cascade across the screen.
