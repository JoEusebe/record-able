...
public final class ExportSettingsScreen extends Screen {
    ...
    public ExportSettingsScreen(Screen parent) {
        super(Text.literal("Export Settings"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        var fpsButton = CycleButton.create(colR, y, halfW, WIDGET_HEIGHT,
                Text.literal("FPS: " + getFpsDisplay(config)),
                b -> {
            config.exportFps = nextFps(config.exportFps);
            config.save();
            b.setMessage(Text.literal("FPS: " + getFpsDisplay(config)));
        }, b -> {
            config.exportFps = prevFps(config.exportFps);
            config.save();
            b.setMessage(Text.literal("FPS: " + getFpsDisplay(config)));
        });
        addDrawableChild(fpsButton);
        y += ROW_SPACING + 10;
        ...
        var resetButton = ThemedButton.create(colL, y, innerW, WIDGET_HEIGHT,
            ...
            config.exportFps = 0;
            config.save();
            if (this.client != null) this.client.setScreen(new ExportSettingsScreen(parent));
        });
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // Title
        context.drawCenteredTextWithShadow(this.textRenderer, this.title, this.width / 2, this.panelY + 10, 0xFFFFFF);
    }
