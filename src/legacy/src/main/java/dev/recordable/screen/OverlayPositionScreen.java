...
public final class OverlayPositionScreen extends Screen {
    ...
    public OverlayPositionScreen(Screen parent) {
        super(Text.translatable("screen.recordable.position_editor.title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        addDrawableChild(ButtonWidget.builder(
                Text.translatable("screen.recordable.position_editor.done"), b -> saveAndClose()
        ).dimensions(startX, btnY, btnW, btnH).build());
        addDrawableChild(ButtonWidget.builder(
                Text.translatable("screen.recordable.position_editor.reset_all"), b -> resetAllDefaults()
        ).dimensions(startX + btnW + gap, btnY, btnW, btnH).build());
        addDrawableChild(ButtonWidget.builder(
                Text.literal(panelOpen ? "▶ Panel" : "◀ Panel"), b -> {
                    panelOpen = !panelOpen;
                    b.setMessage(Text.literal(panelOpen ? "▶ Panel" : "◀ Panel"));
                }
        ).dimensions(startX + (btnW + gap) * 2, btnY, btnW, btnH).build());
        addDrawableChild(ButtonWidget.builder(
                Text.translatable("screen.recordable.position_editor.cancel"), b -> cancelAndClose()
    ...
    private static final class OpacityEntry {
        ...
        OpacityEntry(String label, Supplier<Integer> getter, Consumer<Integer> setter) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        // ── Background ──
        // Draw a manual dark background instead of renderBackground() to avoid
        // potential z-ordering / render-state issues on 1.20.0-1.20.4 where the
        // dirt-texture background can cover subsequently batched DrawContext.fill() calls.
        drawPreviewBackground(context);

        RecordableConfig config = RecordableConfig.get();
        if (config == null || this.client == null) { super.render(context, mouseX, mouseY, delta); return; }
        ...
        context.getMatrices().scale(overlayScale, overlayScale, 1.0f);
        ...
        // Header
        context.drawCenteredTextWithShadow(tr, this.title, this.width / 2, 4, HEADER_COLOR);
        String hint = activeResize != ResizeEdge.NONE ? "Drag to resize · Release to confirm"
                : draggedElement != null ? Text.translatable("screen.recordable.position_editor.hint_dragging").getString()
                : Text.translatable("screen.recordable.position_editor.hint_idle").getString();
        context.drawCenteredTextWithShadow(tr, Text.literal(hint), this.width / 2, 15, HINT_COLOR);
        if (overlayScale != 1.0f) {
            String s = "Scale " + Math.round(overlayScale * 100) + "%";
            context.drawCenteredTextWithShadow(tr, Text.literal(s), this.width / 2, 26, 0xFF777744);
        }
        ...
        if (draggedElement != null || activeResize != ResizeEdge.NONE) {
            ...
            if (el != null) {
                ...
                RenderHelper.drawText(context, tr, Text.literal(coords), cx + 2, cy, COORD_COLOR);
            }
        }
    ...
    private void drawGridGuides(DrawContext context) {
        int cx = vw / 2, cy = vh / 2;
    ...
    private String watermarkPlayerName() {
        return (this.client != null && this.client.player != null)
                ? this.client.player.getName().getString() : "Player";
    }
    ...
    private void buildWatermarkElements(RecordableConfig config, TextRenderer tr) {
        ...
        for (int i = 0; i < config.watermarkSlots.size(); i++) {
            ...
            int tw = Math.max(8, Math.round(tr.getWidth(txt) * f));
    ...
    private void renderWatermarkElement(DrawContext ctx, TextRenderer tr, DraggableElement el, int mx, int my) {
        RecordableConfig config = RecordableConfig.get();
        ...
        String txt = tr.trimToWidth(watermarkPreviewText(slot), el.w);
        int color = parseWatermarkColor(slot.textColor, slot.opacity);
        RenderHelper.drawText(ctx, tr, Text.literal(txt), el.x + 2, el.y + 2, color);
        String tag = "\u25A4 " + el.label;
        int lw = tr.getWidth(tag) + 4;
        int lx = el.x, ly = el.y - 10; if (ly < 0) ly = el.y + el.h + 1;
        ctx.fill(lx, ly, lx + lw, ly + 9, LABEL_BG);
        RenderHelper.drawText(ctx, tr, Text.literal(tag), lx + 2, ly + 1, bc);
    }

    private void openWatermarkEditor(int index) {
        if (this.client != null) { this.client.setScreen(new WatermarkScreen(this)); }
    }

    private void rebuildElements(RecordableConfig config, TextRenderer tr) {
        elements.clear();
        ...
        // PLAY/REC
        {
            ...
            el.autoW = Math.max(60, tr.getWidth("PLAY \u25B6") + 12); el.autoH = textH;
        ...
        // Timestamp
        {
            ...
            el.autoW = tr.getWidth("00:12:34") + 8; el.autoH = 12;
        ...
        // SP
        {
            ...
            el.autoW = tr.getWidth("SP") + 8; el.autoH = 12;
        ...
        // Perf
        {
            ...
            int mw = 0; for (String l : pL) mw = Math.max(mw, tr.getWidth(l));
        ...
        // Classic single-panel element (only shown for the Classic overlay style).
        {
            ...
            int maxW = 0; for (String l : lines) maxW = Math.max(maxW, tr.getWidth(l));
        ...
        // Synthwave single-panel element (only shown for the Synthwave overlay style).
        {
            ...
            el.autoW = tr.getWidth("REC 00:12:34") + 22; el.autoH = 16;
            el.w = el.autoW; el.h = el.autoH;
            el.resizable = false;
        ...
        // Mic (always available, independent of overlay style)
        {
            ...
            el.autoW = tr.getWidth(micLabel) + 16; el.autoH = 13;
            el.w = el.autoW; el.h = el.autoH;
            el.resizable = false;
    ...
    private void renderElement(DrawContext ctx, TextRenderer tr, DraggableElement el, int mx, int my, boolean visible) {
        boolean isDragged = el == draggedElement;
        ...
        if (!visible) {
            ...
            int lw = tr.getWidth(label) + 4;
            int lx = el.x, ly = el.y - 10;
            if (ly < 0) ly = el.y + el.h + 1;
            ctx.fill(lx, ly, lx + lw, ly + 9, 0x66000000);
            RenderHelper.drawText(ctx, tr, Text.literal(label), lx + 2, ly + 1, 0x55FF6666);
            return;
        }
        ...
        if (ly < 0) ly = el.y + el.h + 1;
        ctx.fill(lx, ly, lx + lw, ly + 9, LABEL_BG);
        RenderHelper.drawText(ctx, tr, Text.literal(label), lx + 2, ly + 1, bc);
    }

    private static void drawHandle(DrawContext ctx, int x, int y, int s, int c) {
    ...
    private void renderPanel(DrawContext ctx, TextRenderer tr, int mx, int my) {
        RecordableConfig config = RecordableConfig.get();
        ...
        if (sectionLayersOpen) {
            ...
            for (int i = 0; i < shown.size(); i++) {
                ...
                if (y >= py && y + ROW_H <= py + panelH) {
                    ...
                    RenderHelper.drawText(ctx, tr, Text.literal(eyeIcon), eyeX, y + 3, eyeColor);
                    ...
                    String display = icon + " " + tr.trimToWidth(displayName, innerW - 42);
                    RenderHelper.drawText(ctx, tr, Text.literal(display), left + 13, y + 3, nameColor);

                    // Reorder arrows (rightmost)
                    int ax = left + innerW - 14;
                    if (i > 0) {
                        boolean hu = mx >= ax && mx <= ax + 10 && my >= y && my <= y + 7;
                        RenderHelper.drawText(ctx, tr, Text.literal("\u25B2"), ax, y + 0, hu ? 0xFFFFFF44 : 0xFF555555);
                    }
                    if (i < shown.size() - 1) {
                        boolean hd = mx >= ax && mx <= ax + 10 && my >= y + 8 && my <= y + ROW_H;
                        RenderHelper.drawText(ctx, tr, Text.literal("\u25BC"), ax, y + 8, hd ? 0xFFFFFF44 : 0xFF555555);
                    }
                }
                y += ROW_H;
            }
        ...
        if (sectionWatermarksOpen) {
            ...
            if (slots == null || slots.isEmpty()) {
                if (y >= py && y + ROW_H <= py + panelH) {
                    RenderHelper.drawText(ctx, tr, Text.literal("  (none - add below)"), left + 2, y + 3, 0xFF777777);
                }
                y += ROW_H;
            } else {
                for (int i = 0; i < slots.size(); i++) {
                    ...
                    if (y >= py && y + ROW_H <= py + panelH) {
                        ...
                        RenderHelper.drawText(ctx, tr, Text.literal(eyeIcon), eyeX, y + 3, eyeColor);
                        String nm = "\u25A4 " + tr.trimToWidth((slot != null && slot.name != null ? slot.name : "Watermark"), innerW - 42);
                        RenderHelper.drawText(ctx, tr, Text.literal(nm), left + 13, y + 3, en ? 0xFFCCCCCC : 0xFF777777);
                        int ex = left + innerW - 22;
                        boolean ehov = mx >= ex && mx <= ex + 20 && my >= y && my < y + ROW_H;
                        RenderHelper.drawText(ctx, tr, Text.literal("Edit"), ex, y + 3, ehov ? 0xFFFFCC44 : 0xFF8899BB);
                    }
                    y += ROW_H;
                }
            }
            if (y >= py && y + ROW_H <= py + panelH) {
                ...
                RenderHelper.drawText(ctx, tr, Text.literal("\u2795 Open Watermark Editor"), left + 4, y + 3, ohov ? 0xFF66CCFF : 0xFF6699CC);
            }
            y += ROW_H;
        }
    }
    ...
    private int renderSectionHeader(DrawContext ctx, TextRenderer tr, String openLabel, String closedLabel,
        ...
        if (y >= clipTop && y + ROW_H <= clipBot) {
            ...
            RenderHelper.drawText(ctx, tr, Text.literal(open ? openLabel : closedLabel), x + 5, y + 4, tAccent());
        }
        return y + ROW_H;
    }

    private void renderOpacityRow(DrawContext ctx, TextRenderer tr, OpacityEntry entry,
                                   int x, int y, int w, int mx, int my) {
        int val = entry.getter.get();
        // Label + value
        String lbl = entry.label + " " + val + "%";
        RenderHelper.drawText(ctx, tr, Text.literal(lbl), x + 2, y + 1, 0xFFAAAAAA);
    ...
    private void renderClassicPreview(DrawContext context, RecordableConfig config, TextRenderer tr) {
        ...
        int ly = y;
        RenderHelper.drawText(context, tr, Text.literal(lines[0]), x + 13, ly, 0xFFFFFFFF); ly += 12;
        RenderHelper.drawText(context, tr, Text.literal(lines[1]), x, ly, skin != null ? skin.textSecondary : 0xFFE0E0E0); ly += 11;
        RenderHelper.drawText(context, tr, Text.literal(lines[2]), x, ly, 0xFFFFFFFF); ly += 11;
        RenderHelper.drawText(context, tr, Text.literal(lines[3]), x, ly, 0xFF9BE28F);
    }

    /** Preview the Synthwave single-panel overlay at its (possibly dragged) position. */
    private void renderSynthwavePreview(DrawContext context, RecordableConfig config, TextRenderer tr) {
        ...
        int pw = tr.getWidth(line) + 22, ph = 16;
    ...
    private void renderOverlayPreview(DrawContext context, RecordableConfig config, TextRenderer tr) {
        ...
        if (config.hudPlayRecVisible) {
            if (config.vhsShowPlay)
                RenderHelper.drawText(context, tr, Text.literal("PLAY \u25B6"), leftInset, topInset, playColor);

            int recY = topInset + (config.vhsShowPlay ? 12 : 0);
            context.fill(leftInset, recY + 2, leftInset + 7, recY + 9, recDotColor);
            RenderHelper.drawText(context, tr, Text.literal("REC"), leftInset + 10, recY, recTextColor);
        }

        // Timestamp preview
        if (config.hudTimestampVisible) {
            String timer = "00:12:34"; int timerW = tr.getWidth(timer);
            RenderHelper.drawText(context, tr, Text.literal(timer), vw - config.hudTimestampOffsetX - timerW, config.hudTimestampY, timestampColor);
        }
        ...
        if (config.hudSpVisible && config.vhsShowSp)
            RenderHelper.drawText(context, tr, Text.literal("SP"), config.hudSpX, vh - config.hudSpOffsetY, spColor);
        ...
        if (config.hudDetailsVisible) {
            ...
            if (config.vhsShowDate) {
                ...
                detY -= 22;
                RenderHelper.drawText(context, tr, Text.literal(tl), brX - tr.getWidth(tl), detY, dateColor);
                detY += 11;
                RenderHelper.drawText(context, tr, Text.literal(dl), brX - tr.getWidth(dl), detY, dateColor);
                detY = vh - config.hudDetailsOffsetY - 26;
            } else { detY -= 4; }
            if (config.vhsShowTapeCounter) {
                String tc = "TC 0143"; detY -= 11;
                RenderHelper.drawText(context, tr, Text.literal(tc), brX - tr.getWidth(tc), detY,
            ...
            if (config.vhsShowBattery) {
                ...
                RenderHelper.drawText(context, tr, Text.literal("98%"), bx + bw + 5, detY + 1, 0xFFCCCCCC);
            }
        }
        ...
        if (config.hudPerfVisible && config.showPerformanceStats) {
            ...
            int mw = 0; for (String l : lines) mw = Math.max(mw, tr.getWidth(l));
            ...
            for (int i = 0; i < lines.length; i++) {
                RenderHelper.drawText(context, tr, Text.literal(lines[i]), ppx + 2, ly, RecordableConfig.applyOpacity(cs[i], po));
                ly += 10;
            }
        }
    }

    private static void drawCornerBrackets(DrawContext ctx, int x1, int y1, int x2, int y2, int len, int t, int c) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int mx = (int) mouseX, my = (int) mouseY, btn = button;
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        ...
        // Opacity slider drag
        if (draggingOpacity != null && button == 0) {
            int px = this.width - PANEL_W - 4;
        ...
        // Resize
        if (activeResize != ResizeEdge.NONE && resizeElement != null && button == 0) {
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        ...
        if (activeResize != ResizeEdge.NONE && button == 0) {
    ...
    @Override public void close() { cancelAndClose(); }
