...
// RenderTickCounter removed - 1.20.x HudRenderCallback uses (DrawContext, float)
...
public final class RecordingOverlay {
     ...
     *
     * <p>The HudRenderCallback signature differs between versions:</p>
     * <ul>
     *   <li>1.21+: {@code (DrawContext, RenderTickCounter)}</li>
     *   <li>1.20.x: {@code (DrawContext, float)}</li>
     * </ul>
     * <p>We compile against the 1.21+ API. On 1.20.x, Fabric API's event
     * system will adapt the lambda at the interface level. The render method
     * accepts the 1.21+ signature directly. For 1.20.x compatibility, use
     * {@link #registerCompat()} which registers via reflection if needed.</p>
     */
    public static void register() {
        if (registered) return;
        registered = true;
        // 1.20.x HudRenderCallback uses (DrawContext, float) signature
        HudRenderCallback.EVENT.register(RecordingOverlay::render);
        // Screen-level censor overlay: the HUD callback above does NOT draw while a
        // GUI screen (e.g. inventory) is open, so we also draw the live censor over
    ...
    private static void renderMousePointer(DrawContext context, MinecraftClient client,
        ...
        float[] tint = resolvePointerTint(config.mousePointerTheme);
        RenderSystem.setShaderColor(tint[0], tint[1], tint[2], tint[3]);
        try {
            context.drawTexture(OS_CURSOR_TEXTURE, drawX, drawY, 0.0f, 0.0f,
    ...
    private static float[] resolvePointerTint(RecordableConfig.MousePointerTheme theme) {
        if (theme == null) {
            return new float[] {1.0f, 1.0f, 1.0f, 1.0f};
        }
        return switch (theme) {
            case EDGY_DARK -> new float[] {0.18f, 0.18f, 0.18f, 1.0f};
            case BOOTLEG_TOY_PHONE -> new float[] {1.0f, 0.41f, 0.71f, 1.0f};
            case CLASSIC -> new float[] {1.0f, 1.0f, 1.0f, 1.0f};
        };
    }
    ...
    private static void renderMicIndicator(DrawContext context, MinecraftClient client) {
        ...
        // suppress unused-warning style note: ptt influences only label text above
        if (ptt && live) {
            // brighter underline when actively transmitting in PTT mode
    ...
    private static void renderToastNotification(DrawContext context, MinecraftClient client,
        ...
        for (ToastQueue.Entry entry : toasts) {
            ...
            for (net.minecraft.text.OrderedText line : lines) {
                context.drawText(tr, line, textX, textY, withAlpha(textColor, alpha), false);
                textY += lineHeight;
            }
    ...
    private static void renderImageWatermark(DrawContext context, MinecraftClient client,
        ...
        ms.translate(x, y, 0);
        if (slot.rotation != 0) {
            ms.translate(scaledW / 2.0, scaledH / 2.0, 0);
            ms.multiply(new org.joml.Quaternionf().rotateZ((float) Math.toRadians(slot.rotation)));
            ms.translate(-scaledW / 2.0, -scaledH / 2.0, 0);
        }
        ms.scale(scale, scale, 1.0f);
        com.mojang.blaze3d.systems.RenderSystem.setShaderColor(1.0f, 1.0f, 1.0f, finalAlpha / 255.0f);
        context.drawTexture(tex.id, 0, 0, 0.0f, 0.0f, tex.width, tex.height, tex.width, tex.height);
